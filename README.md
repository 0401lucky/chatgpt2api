...
- 兼容 `POST /v1/search`，按源作者链路调用网页版 `gpt-5-5` 搜索模型
...
- 支持 `gpt-image-2`、`codex-gpt-image-2`、`auto`、`gpt-5`、`gpt-5-1`、`gpt-5-2`、`gpt-5-3`、`gpt-5-3-mini`、`gpt-5-mini`、`gpt-5-5` 模型选择
- 编辑模式支持参考图上传
- 前端支持多图生成交互
...
| 字段   | 说明                                                                                                         |
|:-----|:-----------------------------------------------------------------------------------------------------------|
| 返回模型 | `gpt-image-2`、`codex-gpt-image-2`、`auto`、`gpt-5`、`gpt-5-1`、`gpt-5-2`、`gpt-5-3`、`gpt-5-3-mini`、`gpt-5-mini`、`gpt-5-5` |
...
网页版 `gpt-5-5` 搜索接口，返回搜索答案、来源和 conversation id。
...
curl http://localhost:3000/v1/search \
  ...
  -H "Authorization: Bearer <auth-key>" \
  -d '{
    "prompt": "今天 OpenAI 有哪些新消息？"
  }'
```

也可以在 `/v1/chat/completions` 中使用 `model: "gpt-5-5"`，Go 后端会自动转到同一条搜索链路并返回文本答案。
