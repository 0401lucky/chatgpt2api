...
结论先说：**可以用 Go 重写后端，当前仓库已经把默认 Docker
入口切到了 Go 后端**。
早期建议是先做旁路 Go 核心服务；现在这条路线已经推进到单容器双进程形态。
后续重点不再是“能不能切”，而是继续补齐深度迁移、真实上游验证和大账号池压测。
...
- 根目录 `Dockerfile` 与 `docker-compose.yml` 已切到单容器双进程形态。
...
- `GET /api/image-tasks`、`GET /api/creation-tasks`、
  `POST /api/image-tasks/generations`、`POST /api/image-tasks/edits`、
  `POST /api/creation-tasks/image-generations`、
  `POST /api/creation-tasks/image-edits` 已接入 Go。
- `GET /v1/models`、`POST /v1/chat/completions`、`POST /v1/responses`、
  `POST /v1/messages`、`POST /v1/images/generations`、
  `POST /v1/images/edits` 已接入 Go。
- `POST /v1/images/generations` 已支持 `response_format=url`
  和 `response_format=b64_json`。
- `/api/creation-tasks`、`/api/creation-tasks/image-generations`
  和 `/api/creation-tasks/image-edits`
  已作为兼容别名接入。
- `gpt-image-2` 当前在 Go 图片链路中映射为 `gpt-5-3`。
- Turnstile 已修，真实 live test 可解出可用 token。
- 图生图 / 图片编辑已迁移到 Go，但仍需要真实账号补充更多上游场景验证。
- `go test ./...` 已通过。
...
下面这段是早期旁路阶段设计。当前根 `Dockerfile` 与
`docker-compose.yml` 已经切到 Go 单容器双进程形态。

- 早期不替换现有 Python/Web 容器。
- 早期不修改当前 Dockerfile。
- 早期让用户可以在 Docker Desktop 中保持 Go 后端运行，
...
这是前一轮 Docker Desktop 可用时的记录。当前这轮用户机器上的 Docker
Desktop 卡住，因此本轮只做本地 Go 进程验证。
...
- `gpt-image-2` 到 `gpt-5-3` 的模型映射
- 真实账号图片生成任务回填和下载结果处理

当前边界：

- `POST /api/image-tasks/edits` 已接入 Go。
- `POST /api/creation-tasks/image-edits` 已接入 Go。
- `POST /v1/images/edits` 已接入 Go。
- 图生图仍需要更多真实账号和真实上游场景验证。
...
本节是历史阶段设计。当前已经从旁路验证推进到默认 Docker 入口，
保留本节用于解释为什么先做 Go 核心服务、再逐步扩大覆盖面。
...
当前状态：根 `Dockerfile`、`docker-compose.yml` 和主要管理接口已经推进到
这一阶段。下面工作项保留为阶段目标记录，其中 Dockerfile 与 compose
改造已经完成。
...
3. Dockerfile 与 docker-compose 改造。当前已完成。
4. 增加运行模式：
   - `backend=python`
   - `backend=go`
   - `backend=hybrid`
...
本节是历史阶段回滚设计。当前默认 Docker 入口已经是 Go 后端；
如果后续需要回滚，应基于 Git 版本或旧 Python 镜像回退，而不是在同一个
compose 中默认保留两个服务。
...
4. 接入 R2 真实远端上传和恢复链路。
5. 深迁图床自动上传替换链路。
6. 评估并迁移 Python 版可编辑文件任务能力，优先关注 PPT，其次再看 PSD。
7. 补完 Responses、Messages 的完整流式事件和更多兼容细节。
8. 清理旧 Python 路径和历史部署说明，只保留必要回滚文档。


## ✅ 最终建议

Go 重写值得做，当前也已经进入默认 Docker 入口阶段。

最推荐的下一步是：

1. 用真实测试账号完成账号刷新、文本、文生图、图生图全链路验证。
2. 用同一批账号和同一组请求做 Python / Go A/B 对比。
3. 如果 Go 在核心链路上确实明显更稳、更省资源，再继续深迁注册、R2、图床等周边能力。
4. 部署文档继续以单容器双进程为主，历史旁路方案只作为回滚参考。
