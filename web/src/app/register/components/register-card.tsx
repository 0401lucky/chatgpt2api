                ...
                return (
                  <div key={index} className="space-y-3 border-t border-stone-200 pt-3 first:border-t-0 first:pt-0">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      {type === "cloudflare_temp_email" || type === "tempmail_plus" || type === "moemail" || type === "inbucket" || type === "yyds_mail" || type === "ddg_mail" || type === "cloudmail_gen" ? (
                        <>
                          <div className="space-y-2">
                    ...
                    {type === "tempmail_lol" || type === "tempmail_plus" || type === "cloudflare_temp_email" || type === "moemail" || type === "inbucket" || type === "duckmail" || type === "yyds_mail" || type === "ddg_mail" || type === "cloudmail_gen" ? (
                      <div className="space-y-2">
                        <label className="text-sm text-stone-700">{type === "cloudmail_gen" ? "邮箱域名" : type === "inbucket" ? "基础域名列表" : "Domain"}</label>
                        <Textarea value={domains} onChange={(event) => updateProvider(index, { domain: event.target.value.split(/[\n,]/).map((item) => item.trim()).filter(Boolean) })} placeholder={type === "cloudmail_gen" ? "每行一个域名，留空则使用服务默认域名" : type === "inbucket" ? "每行一个基础域名，系统会自动生成随机子域名" : type === "duckmail" ? "每行一个域名，随机选择；留空则读取 DuckMail /domains" : type === "tempmail_plus" ? "每行一个域名，留空则使用 TempMail.Plus 默认域名池" : type === "moemail" ? "每行一个域名" : "每行一个域名，留空则使用服务默认域名"} className="min-h-20 rounded-xl border-stone-200 bg-white font-mono text-xs" disabled={config.enabled} />
                      </div>
                    ) : null}
                    {type === "cloudmail_gen" ? (
                      <div className="space-y-2">
